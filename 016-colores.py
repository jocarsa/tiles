...
# Apply random color multiplication
...
for frame_idx in range(frame_count):
    # Create a blank frame with the random background color
    output_frame = np.full((frame_height, frame_width, 3), [int(c * 255) for c in random_color], dtype=np.uint8)
    ...
    for idx, (y_start, x_start) in enumerate(positions):
        ...
        # Adjust output_frame slice size to match cropped_tile size

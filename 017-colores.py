...
# Load the tile
tile_path = 'esquina.png'  # Replace with your tile image path
original_tile = cv2.imread(tile_path)

# Apply random color multiplication to tint the tile
random_color = [random.uniform(0.5, 1.5) for _ in range(3)]
colored_tile = np.clip(original_tile * random_color, 0, 255).astype(np.uint8)

# Compute the average color of the tinted tile for the background
avg_color = cv2.mean(colored_tile)[:3]  # B, G, R channels only
avg_color = [int(c) for c in avg_color]

# Frame dimensions and video settings
frame_width, frame_height = 1920, 1080
fps = 60
duration = 60 * 60  # Duration in seconds for the video
...
tile_size = random.randint(32, 64)  # Random size for all tiles
resized_tile = cv2.resize(colored_tile, (tile_size, tile_size))
...
num_tiles_x = (frame_width // tile_size) + 1
num_tiles_y = (frame_height // tile_size) + 1
...
rotation_directions = [random.choice([-180, -90, 0, 90, 180]) for _ in positions]  # Random initial direction for each tile

# Generate video frames
for frame_idx in range(frame_count):
    # Create a blank frame using the average tinted color as background
    output_frame = np.full((frame_height, frame_width, 3), avg_color, dtype=np.uint8)
    ...
    for idx, (y_start, x_start) in enumerate(positions):
        ...
        if tile_time_counters[idx] < wait_time_frames:
            ...
            # Tile is animating if rotation is not 0
            ...
            if rotation_directions[idx] != 0:
                # Only animate if the direction is not 0
                progress = animation_frame / animation_time_frames
                current_angle = start_angle + (end_angle - start_angle) * progress
            else:
                # If no rotation is chosen, keep the current angle
            ...
            if animation_frame == animation_time_frames - 1:
                ...
                rotation_directions[idx] = random.choice([-180, -90, 0, 90, 180])
        else:
        ...
        # Place the tile onto the frame
        output_frame[y_start:y_start + cropped_tile.shape[0], x_start:x_start + cropped_tile.shape[1]] = cropped_tile
